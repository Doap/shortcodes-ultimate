...
//			echo $dest_file_name . ' does not exist and dir =' . $dir . PHP_EOL;
			$query = $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE guid='%s'", $url );
			$get_attachment = $wpdb->get_results( $query );
			if ( !$get_attachment )
				return array( 'url' => $url, 'width' => $width, 'height' => $height );
			// Load Wordpress Image Editor
			//echo "starting editor with $file_path" . PHP_EOL;
...
//			echo "original width $orig_width    original height $orig_height" . PHP_EOL;
			$src_x = $src_y = 0;
			$src_w = $orig_width;
			$src_h = $orig_height;
			if ( $crop ) {
...
//echo 'dest_file_name = ' . $dest_file_name . PHP_EOL;
...
//echo 's3result = ' . $s3result . PHP_EOL;
...
class Su_Tools {
	...
	public static function get_slides( $args ) {
		...
		$query['date_query'] = array(array('column' => 'post_date', 'after' => '1 month ago'));
...
*/
		...
		//if ( $args['meta'] = 'breves' ) var_dump($query);
		//if ( $args['meta'] = 'notbreves' ) var_dump($query); 
		// Loop through posts
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
		//	echo 'POST ID ' . $post->ID . PHP_EOL;
		//	echo 'POST EXCERPT' . $post->post_excerpt . PHP_EOL;
...
//		echo PHP_EOL . '------------POST EXCERPT------------' . $theexcerpt . PHP_EOL;
		if (!isset($theexcerpt) || empty($theexcerpt))
		{
			$theexcerpt = '';
				...
				//$my_attach_id = get_post_thumbnail_id( $post->ID );
				//$thumb = ( $args['source']['type'] === 'media' ) ? $post->ID : $my_attach_id; 
